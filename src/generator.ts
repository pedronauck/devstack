...
const BASE_CLAUDE_DIR = path.join(TEMPLATES_DIR, "base", "claude");
...
function buildCommitlintConfig() {
  return ["export default {", '  extends: ["@commitlint/config-conventional"],', "};", ""].join(
    "\n"
  );
}

function buildMakefile(context: GenerateContext) {
...
function buildInitTestDbScript(context: GenerateContext) {
  return replaceTemplateTokens(
    [
      "#!/usr/bin/env bash",
      "set -euo pipefail",
...
function buildDockerCompose(context: GenerateContext) {
...
function buildClaudeMd(context: GenerateContext) {
  ...
  return replaceTemplateTokens(
    [
      "# CLAUDE.md",
      "",
      "This file provides guidance for AI coding agents working inside {{projectTitle}}.",
      "",
      "## Commands",
      "",
      "```bash",
      ...
      "## Architecture",
      "",
      "- `packages/frontend`: React 19 + Vite + TanStack Router + TanStack Query",
      "- `packages/backend`: Hono + Drizzle + PostgreSQL",
      "- `src/modules/items`: example backend CRUD module",
      "",
      "## Rules",
      "",
      "- Keep the route/usecase/repository layering intact for backend modules.",
      "- Prefer generic abstractions; this scaffold should stay domain-agnostic.",
      "- Run `make check` before finishing meaningful changes.",
      moduleSections ? `\n${moduleSections}` : "",
    ].join("\n"),
    context.tokens
  );
...
async function patchPluginNames(context: GenerateContext) {
  const files = [
    "lint-plugins/react-component-complexity.mjs",
    "lint-plugins/react-hooks-separation.mjs",
    "lint-plugins/test-file-location.mjs",
...
async function writeDynamicFiles(context: GenerateContext) {
  ...
  await writeTextFile(path.join(context.targetDir, "CLAUDE.md"), buildClaudeMd(context));
...
export async function generate(config: GeneratorConfig): Promise<GenerateResult> {
  ...
  await copyDirectory(BASE_ROOT_DIR, targetDir, tokens);
  await copyDirectory(BASE_FRONTEND_DIR, path.join(targetDir, "packages/frontend"), tokens);
  await copyDirectory(BASE_BACKEND_DIR, path.join(targetDir, "packages/backend"), tokens);
  await copyDirectory(BASE_CLAUDE_DIR, path.join(targetDir, ".claude"), tokens);
  ...
  for (const moduleName of resolvedModules) {
    ...
    if (await pathExists(overlayDir)) {
      await copyDirectory(
        overlayDir,
        await resolveModuleOverlayTargetDir(targetDir, overlayDir),
        tokens
      );
    }
  }
