...
const SKILLS_DIR = path.join(ROOT_DIR, "vendor", "skills", "skills");
...
function resolveSkillsForModules(resolvedModules: ModuleName[]): Set<string> {
...
function buildSkillMappings(context: GenerateContext): SkillMapping[] {
  ...
  return [...BASE_SKILL_MAPPINGS, ...moduleMappings];
}

function buildSkillEnforcementSection(context: GenerateContext) {
  const lines: string[] = [
    "## Skills Enforcement",
    "",
...
function buildClaudeMd(context: GenerateContext) {
  ...
  const skillEnforcement = buildSkillEnforcementSection(context);
...
export async function generate(config: GeneratorConfig): Promise<GenerateResult> {
  ...
  const selectedSkills = resolveSkillsForModules(resolvedModules);
